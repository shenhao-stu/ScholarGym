...
YAML_TO_CONFIG = {
    ...
    "eval_base_dir": ("EVAL_BASE_DIR", lambda v: v),
    "qdrant_url": ("QDRANT_URL", lambda v: v),
    ...
    "save_agent_traces": ("SAVE_AGENT_TRACES", lambda v: bool(v)),
    "save_case_study_artifacts": ("SAVE_CASE_STUDY_ARTIFACTS", lambda v: bool(v)),
    "verbose": ("VERBOSE", lambda v: bool(v)),
    "case_study_output_dir": ("CASE_STUDY_OUTPUT_DIR", lambda v: v),
    "browser_max_tokens": ("BROWSER_MAX_TOKENS", lambda v: int(v)),
    "top_k": ("EVAL_TOP_K_VALUES", lambda v: list(v)),
    "gt_rank_cutoff": ("GT_RANK_CUTOFF", lambda v: int(v)),
    "context_max_length_chars": ("CONTEXT_MAX_LENGTH_CHARS", lambda v: int(v)),
    "enable_llm_filtering": ("ENABLE_LLM_FILTERING", lambda v: bool(v)),
    "llm_filtering_batch_size": ("LLM_FILTERING_BATCH_SIZE", lambda v: int(v)),
    "max_pages_per_query": ("MAX_PAGES_PER_QUERY", lambda v: int(v)),
    "enable_summarization": ("ENABLE_SUMMARIZATION", lambda v: bool(v)),
    "summary_cache_path": ("SUMMARY_CACHE_PATH", lambda v: v),
    "summary_abstract_char_threshold": ("SUMMARY_ABSTRACT_CHAR_THRESHOLD", lambda v: int(v)),
    "summary_model": ("SUMMARY_LLM_MODEL_NAME", lambda v: v),
...
CONFIG_SECTIONS: list[tuple[str, list[tuple[str, str]]]] = [
    ...
    (
        "Debug / tracing",
        [
            ("SAVE_CASE_STUDY_ARTIFACTS", "Save case-study prompts/responses/fulltext for the first 5 samples."),
